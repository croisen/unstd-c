...
functions=(
    ...
    "isize sys_linux_execve(const char *filename, const char *const argv[], const char *const envp[])"
...
for func in "${functions[@]}"; do
    syscall_macro="$(echo "$func" | sed "s/isize \([a-z0-9_]*\).*/\1/")"
    filename="$(printf "%03d_%s.c\n" "$start_num" "$syscall_macro" | sed "s/sys_linux_//")"
    arguments="$(echo "$func" | sed "s/isize [a-z0-9_]*(\(.*\))/\1/")"
    arguments="$(echo "$arguments" | awk -F", " '{ for (i=1;i<=NF;i++) { n=split($i, a," "); printf "%s ", a[n] } }' )"
    arguments="$(echo "$arguments" | awk -v OFS=", " '{ $1=$1; print }' )"
    if [ -n "$arguments" ] && [ "$arguments" != "void" ]; then
    ...
    printf "$filename\n"
    printf "#include \"integer.h\"\n#include \"sys/linux/syscall.h\"\n\n%s {\n    %s;\n}\n" \
        "$func" \
        "return __syscall(__$syscall_macro$arguments)" \
    > "src/sys/linux/$filename"
